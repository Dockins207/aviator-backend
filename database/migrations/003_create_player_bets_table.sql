...
DECLARE
    ...
    v_current_time TIMESTAMP WITH TIME ZONE;
BEGIN
    -- Automatically transition from betting to in_progress after 5 seconds
    IF NEW.status = 'betting' THEN
        v_session_start_time := NEW.created_at;
        v_current_time := CURRENT_TIMESTAMP;
        v_betting_window_end := v_session_start_time + INTERVAL '5 seconds';

        IF v_current_time >= v_betting_window_end THEN
    ...
    IF NEW.status = 'in_progress' AND NEW.crash_point_history IS NOT NULL THEN
